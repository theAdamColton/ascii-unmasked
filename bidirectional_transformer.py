...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, dim=768, heads=8):
        ...
        self.self_attention_heads = nn.ModuleList(
            [Attention(dim, heads) for _ in range(heads)]
        )
...
class BidirectionalTransformer(nn.Module):
    def __init__(
        ...
        self.pos_emb = nn.init.trunc_normal_(
            nn.Parameter(torch.zeros(self.num_image_tokens * 2, dim)), 0.0, 0.02
        )
        # self.register_buffer("pos_emb", nn.init.trunc_normal_(nn.Parameter(torch.zeros(1024, args.dim)), 0., 0.02))
        self.blocks = nn.Sequential(
            *[Encoder(dim, hidden_dim) for _ in range(n_layers)]
        )
        ...
        self.bias = nn.Parameter(
            torch.zeros(self.num_image_tokens * 2, num_codebook_vectors + 2)
        )
    ...
    def forward(self, x):
        ...
        logits = (
            torch.matmul(embed, self.tok_emb.weight.T) + self.bias[: embed.shape[1], :]
        )
